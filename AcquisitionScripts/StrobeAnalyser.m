%% Analysing Strobe
function [StrobeLocSamples, StrobeSignal, StrobeLocPlotterSamples] = StrobeAnalyser(StrobeSignal,sampr,fps,plotStrobe, SPATERN)
%test
...
minInbStr = (0.5*((SPATERN/fps) - 32E-3))*sampr;
if crossHighFall(1) > crossHigh(1)
    for e=1:(min([numel(crossHighFall), numel(crossHigh)]) - 1)
        if ((crossHighFall(e) - crossHigh(e)) < (sampr*20E-3))||((crossHigh(e+1) - crossHighFall(e)) < minInbStr) %strobe pulse is less than 20ms
...
elseif crossHighFall(1) < crossHigh(1)
    for e=1:(min([numel(crossHighFall), numel(crossHigh)])-1)
        if ((crossHighFall(e+1) - crossHigh(e)) < (sampr*20E-3))||((crossHigh(e) - crossHighFall(e)) < minInbStr)  %strobe pulse is less than 20ms
